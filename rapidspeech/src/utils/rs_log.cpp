...
static const char *rs_log_level_to_str(RSLogLevel level) {
  switch (level) {
  case RSLogLevel::RS_LOG_LEVEL_DEBUG:
    return "DEBUG";
  case RSLogLevel::RS_LOG_LEVEL_INFO:
    return "INFO ";
  case RSLogLevel::RS_LOG_LEVEL_WARN:
    return "WARN ";
  case RSLogLevel::RS_LOG_LEVEL_ERR:
    return "ERROR";
  default:
    return "UNKNOWN";
  }
...
void rs_log_set_level(RSLogLevel level) { g_current_log_level = level; }

...
#define LLM_MAX_EXPERTS 512 // Qwen3 Next

struct llm_hparams_posnet {
  uint32_t n_embd;
  uint32_t n_layer;
};
...
struct rapid_hparams {
  ...
  //     -1; // if non-negative, the first n_layer_kv_from_start layers have KV
  //         // cache
  uint32_t n_rot;
  uint32_t n_embd_head_k; // dimension of keys (d_k). d_q is assumed to be the
                          // same, but there are n_head q heads, and only
                          // n_head_kv k-v heads
  ...
  // enum llama_rope_scaling_type rope_scaling_type_train =
  //     LLAMA_ROPE_SCALING_TYPE_NONE;
  //
  // // Step35: optional per-layer clamps for (Swi)GLU
  // std::array<float, LLAMA_MAX_LAYERS>
  //     swiglu_clamp_exp; // clamping for expert FFN
  ...
  // // corresponds to Mamba's conv_states size or RWKV's token_shift states
  // size uint32_t n_embd_r() const;
  //
  ...
  // // hparams note: inlined on purpose for performance reasons
...
static_assert(std::is_trivially_copyable<llm_hparams>::value,
              "llama_hparams must be trivially copyable");

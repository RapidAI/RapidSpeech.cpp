...
def main():
    ...
    bert_emb = state["bert"].get("module.embeddings.word_embeddings.weight")
    if bert_emb is None:
        bert_emb = state["bert"].get("embeddings.word_embeddings.weight")
    if bert_emb is None:
    ...
    # Only ConvTranspose1d in Kokoro lives at `dec.gen.ups.N.weight` — keep it
    # as 3D so the Metal upsample kernel and `kokoro_convt1d_pad` continue to
    # read the (K, Cout, Cin) layout they expect. Everything else with a 3D
    # `.weight` is a Conv1d, and we flatten it to 2D [Cin*K, Cout] so that
    # ne[0] = Cin*K is large enough for k-quants to engage instead of being
    # demoted to F16 by `pick()` alignment fallback.
    _convt1d_re = re.compile(r"^dec\.gen\.ups\.\d+\.weight$")
    ...
    for component in ("bert", "bert_encoder", "predictor", "decoder", "text_encoder"):
        ...
        for k in sorted(comp_state.keys()):
            ...
            if arr.ndim <= 1:
                ...
                if arr.ndim == 3 and not _convt1d_re.match(gn):
                    # PyTorch Conv1d weight is (Cout, Cin, K). Flatten the
                    # last two dims so the GGUF tensor has ggml shape
                    # [Cin*K, Cout] (ne[0]=Cin*K, ne[1]=Cout) — matches the
                    # internal reshape ggml_conv_1d already does at runtime,
                    # but lets the quantizer see a 2D matrix with a large
                    # ne[0]. Geometry (K, Cin) is reconstructed in the C++
                    # graph builder from the model config, so no metadata
                    # is needed in the GGUF.
                    Cout, Cin, K = arr.shape
                    arr = arr.reshape(Cout, Cin * K)
                    n_conv_flat += 1
    ...
    print(f"\nMapped: {n_mapped}, skipped: {n_skipped}, conv1d 3D→2D flattened: {n_conv_flat}")
